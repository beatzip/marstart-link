...
pub fn parse_wireguard_config(text: &str) -> Result<ParsedConfig, String> {
    ...
    for (line_num, raw_line) in text.lines().enumerate() {
        ...
        if line.starts_with('[') && line.ends_with(']') {
            ...
            match line {
                "[Interface]" => { in_interface = true; in_peer = false; }
                "[Peer]" => { in_interface = false; in_peer = true; current_peer = Some(ParsedPeerBuilder::new()); }
        ...
        if in_interface {
            ...
            let builder = current_peer.as_mut().ok_or_else(|| format!("Line {}: Peer field outside [Peer]", line_num + 1))?;
            match key {
                "PublicKey" => builder.public_key = Some(decode_wg_key(value, "PublicKey", line_num)?),
                "PresharedKey" => builder.preshared_key = Some(decode_wg_key(value, "PresharedKey", line_num)?),
                "Endpoint" => builder.endpoint = Some(parse_endpoint(value, line_num)?),
                "PersistentKeepalive" => builder.persistent_keepalive = Some(value.parse().map_err(|e| format!("Line {}: Invalid Keepalive: {}", line_num + 1, e))?),
                "AllowedIPs" => {
                    for ip_cidr in value.split(',') {
                        let trimmed = ip_cidr.trim();
                        if !trimmed.is_empty() { builder.allowed_ips.push(parse_allowed_ip(trimmed, line_num)?); }
                    }
                }
                _ => {}
            }
...
fn decode_wg_key(s: &str, field_name: &str, line_num: usize) -> Result<[u8; WIREGUARD_KEY_LENGTH], String> {
    let bytes = general_purpose::STANDARD.decode(s).map_err(|e| format!("Line {}: Invalid {} base64: {}", line_num + 1, field_name, e))?;
    if bytes.len() != WIREGUARD_KEY_LENGTH { return Err(format!("Line {}: {} must be 32 bytes", line_num + 1, field_name)); }
...
fn parse_endpoint(s: &str, line_num: usize) -> Result<SocketAddr, String> {
    ...
    let mut addrs = s.to_socket_addrs().map_err(|e| format!("Line {}: Failed to resolve '{}': {}", line_num + 1, s, e))?;
    addrs.next().ok_or_else(|| format!("Line {}: Endpoint '{}' resolved to no addresses", line_num + 1, s))
}

fn parse_allowed_ip(s: &str, line_num: usize) -> Result<ParsedAllowedIp, String> {
    let Some((ip_str, cidr_str)) = s.split_once('/') else { return Err(format!("Line {}: Invalid AllowedIP '{}'", line_num + 1, s)); };
    let address: IpAddr = ip_str.parse().map_err(|e| format!("Line {}: Invalid IP '{}': {}", line_num + 1, ip_str, e))?;
    let cidr: u8 = cidr_str.parse().map_err(|e| format!("Line {}: Invalid CIDR '{}': {}", line_num + 1, cidr_str, e))?;
    if cidr > (if address.is_ipv4() { 32 } else { 128 }) { return Err(format!("Line {}: CIDR too large", line_num + 1)); }
...
impl ParsedPeerBuilder {
    fn new() -> Self { Self { public_key: None, preshared_key: None, endpoint: None, persistent_keepalive: None, allowed_ips: Vec::new() } }
    fn build(self, line_num: usize) -> Result<ParsedPeer, String> {
        let public_key = self.public_key.ok_or_else(|| format!("Line {}: Peer missing PublicKey", line_num))?;
        Ok(ParsedPeer { public_key, preshared_key: self.preshared_key, endpoint: self.endpoint, persistent_keepalive: self.persistent_keepalive, allowed_ips: self.allowed_ips })
    }
}