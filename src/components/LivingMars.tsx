...
const PARAMS = {
  offline:    { br:0.09, ra:0.00, ps:0.00, ai:0.09, oa:0.04, ml:0.00, warn:null      },
...
// ══════════════════════════════════════════════════════════════════════════
function LivingMars({ state = 'offline' }) {
...
// ══════════════════════════════════════════════════════════════════════════
//  DEMO APPLICATION
//  Design language: Linear · Arc · Stripe — expensive, silent, deliberate
// ══════════════════════════════════════════════════════════════════════════

// Simulated metrics per state
const METRICS = {
  offline:    { ping:'—',    jitter:'—',   loss:'—',    quality:'—',         traffic:'—'         },
  connecting: { ping:'…',    jitter:'…',   loss:'…',    quality:'PROBING',   traffic:'—'         },
  connected:  { ping:'48',   jitter:'3.8', loss:'0.0',  quality:'GOOD',      traffic:'8.2 MB/s'  },
  excellent:  { ping:'22',   jitter:'1.1', loss:'0.0',  quality:'PRIME',     traffic:'12.4 MB/s' },
  warning:    { ping:'148',  jitter:'24',  loss:'0.8',  quality:'DEGRADED',  traffic:'4.1 MB/s'  },
  critical:   { ping:'280',  jitter:'82',  loss:'6.4',  quality:'CRITICAL',  traffic:'1.2 MB/s'  },
};
const ENDPOINT = {
  connected:  'Frankfurt, EU  ·  185.25.48.12',
  excellent:  'Amsterdam, EU  ·  172.67.181.14',
  warning:    'Tokyo, JP  ·  203.0.113.1',
  critical:   'Mumbai, IN  ·  198.18.0.1',
};
const QUALITY_COLOR = {
  PRIME: '#D08470', GOOD: '#A87060', DEGRADED: '#B88030',
  CRITICAL: '#C03828', PROBING: '#4C4468', '—': '#22202C',
};
const DATA_COLOR = {
  offline:'#22202C', connecting:'#362E42', connected:'#A06858',
  excellent:'#C07866', warning:'#B08838', critical:'#C03828',
};

export default function App() {
  const [state,   setState]   = useState('connecting'); // boot with animation
  const timerRef = useRef(null);
  const isConnected  = !['offline','connecting'].includes(state);
  ...
  useEffect(() => {
    ...
    el.textContent = `
      ...
      *{box-sizing:border-box;margin:0;padding:0}
      body{background:${T.space}}
      input[type=range]{-webkit-appearance:none;height:2px;background:rgba(255,255,255,0.05);border-radius:1px;outline:none;width:100%}
      input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;width:10px;height:10px;border-radius:50%;background:#C03028;cursor:pointer}
      @keyframes ring-pulse{0%{transform:scale(1);opacity:.5}100%{transform:scale(1.14);opacity:0}}
  ...
  // Boot sequence: connecting → connected → excellent
  useEffect(() => {
    const t1 = setTimeout(() => setState('connected'),  3400);
    const t2 = setTimeout(() => setState('excellent'),  5600);
  ...
  const handleConnect = useCallback(() => {
    ...
    if (isConnected) {
      ...
      timerRef.current = setTimeout(() => {
        setState('connected');
        timerRef.current = setTimeout(() => setState('excellent'), 2200);
      }, 3400);
    }
  ...
  const m   = METRICS[state]   ?? METRICS.offline;
  const ep  = ENDPOINT[state]  ?? '—';
  const dc  = DATA_COLOR[state] ?? DATA_COLOR.offline;

  // Status indicator text & colour
  const STATUS = {
    offline:    { txt: 'Offline',       dot: '#2A2438' },
    connecting: { txt: 'Connecting',    dot: '#6040B8', anim:'dot-blink 1.4s ease-in-out infinite' },
    connected:  { txt: 'Connected',     dot: '#6C8C6C' },
    excellent:  { txt: 'Prime Signal',  dot: '#8CA87C' },
    warning:    { txt: 'High Latency',  dot: '#9A7830' },
    critical:   { txt: 'Packet Loss',   dot: '#9A2820' },
  };
  const st = STATUS[state] ?? STATUS.offline;

  // Connect button visual config
  const BTN = {
    offline:    { label:'Connect',       bg:'#0A0A14', border:'#1C1828', color:'#3C3450', shadow:'transparent' },
    connecting: { label:'Connecting',    bg:'#0E0810', border:'#3A1422', color:'#6A3040', shadow:'rgba(180,30,25,0.30)' },
    connected:  { label:'Disconnect',    bg:'#0E0810', border:'#4A1820', color:'#B05050', shadow:'rgba(180,40,28,0.18)' },
    excellent:  { label:'Disconnect',    bg:'#0E0810', border:'#5C1C22', color:'#C06060', shadow:'rgba(198,46,30,0.24)' },
    warning:    { label:'Disconnect',    bg:'#0E0C0A', border:'#4A3010', color:'#A07030', shadow:'rgba(160,95,15,0.18)' },
    critical:   { label:'Reconnect',     bg:'#100808', border:'#4A1010', color:'#B03028', shadow:'rgba(175,28,20,0.35)' },
  };
  const btn = BTN[state] ?? BTN.offline;

  // Stat cards
  const statCards = [
    { key:'ping',    label:'PING',    val:m.ping,    unit:'ms' },
    { key:'jitter',  label:'JITTER',  val:m.jitter,  unit:'ms' },
    { key:'loss',    label:'LOSS',    val:m.loss,    unit:'%'  },
    { key:'quality', label:'ROUTE',   val:m.quality, unit:''   },
  ];

  // Simulation state buttons (demo-only, visually demoted)
  const SIM_STATES = [
    ['offline','Offline'],['connecting','Connecting'],['connected','Connected'],
    ['excellent','Excellent'],['warning','Latency'],['critical','Packet Loss'],
  ];
  ...
  return (
    <div style={{
      ...
      {/* ── App header ─────────────────────────────────────────────── */}
      <div style={{
        display:'flex', alignItems:'center', gap:10, marginBottom:10, width:480,
      }}>
        {/* ML logotype mark */}
        <div style={{
          width:30, height:30, borderRadius:8, flexShrink:0,
          background:'linear-gradient(145deg, #8A1A12 0%, #C42E1E 100%)',
          display:'flex', alignItems:'center', justifyContent:'center',
          fontSize:9, fontWeight:700, letterSpacing:'0.06em',
          color:'rgba(252,226,210,0.90)',
        }}>
          ML
        </div>
        <div style={{ flex:1 }}>
          <div style={{ fontSize:14, fontWeight:600, letterSpacing:'0.06em', color:'#C8C4D8' }}>
            Marstart Link
          </div>
          <div style={{ fontSize:9, color:T.txt3, letterSpacing:'0.22em', marginTop:1 }}>
        ...
        <div style={{
          ...
          fontSize:9, color:T.txt2, letterSpacing:'0.14em',
        }}>
          <div style={{
      ...
      <div style={{ position:'relative', marginTop:-6, display:'flex', justifyContent:'center' }}>
        ...
        {isConnecting && (
          <div style={{
            ...
            border:'1px solid rgba(174,36,26,0.50)',
            animation:'ring-pulse 2s cubic-bezier(0.25,0.46,0.45,0.94) infinite',
            pointerEvents:'none',
          }} />
        )}
        <button
          ...
          style={{
            width: 244, height: 44, borderRadius: 10,
            background: btn.bg,
      ...
      <div style={{
        display:'grid', gridTemplateColumns:'1fr 1fr', gap:6, width:480, marginTop:14,
      }}>
        {statCards.map(card => (
          <div key={card.key} style={{
            ...
            <div style={{
              fontSize:8, letterSpacing:'0.28em', color:T.txt3,
            ...
            <div style={{
              ...
              fontSize: card.key === 'quality' ? 11 : 20,
              fontWeight: card.key === 'quality' ? 500 : 400,
              color: card.key === 'quality' ? (QUALITY_COLOR[card.val]??T.txt3) : dc,
              letterSpacing: card.key === 'quality' ? '0.08em' : '-0.01em',
              lineHeight: 1,
            }}>
              {card.val}
              {card.unit && (
                <span style={{ fontSize:9, color:T.txt3, marginLeft:3, fontWeight:400 }}>
                  {card.unit}
                </span>
              )}
            </div>
      ...
      {/* ── Active endpoint ────────────────────────────────────────── */}
      <div style={{
        width:480, marginTop:6,
        background:'rgba(10,10,18,0.90)',
        border:`1px solid ${T.border}`,
        borderRadius:8, padding:'10px 16px',
        display:'flex', alignItems:'center', justifyContent:'space-between',
      }}>
        <div style={{ fontSize:8, letterSpacing:'0.28em', color:T.txt3, fontWeight:500 }}>
          ACTIVE ENDPOINT
        </div>
        <div style={{
          fontFamily:'"JetBrains Mono","SF Mono",monospace',
          fontSize:11, color:isConnected?dc:T.txt3, letterSpacing:'0.02em',
        }}>
          {ep || '—'}
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{
          fontFamily:'"JetBrains Mono","SF Mono",monospace',
          fontSize:11, color:isConnected?dc:T.txt3,
        }}>
          {m.traffic}
        </div>
      </div>
      ...
      <div style={{ width:480, marginTop:28, paddingTop:16, borderTop:`1px solid ${T.border}` }}>
        <div style={{
          fontSize:8, letterSpacing:'0.30em', color:T.txt3,
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:5 }}>
          {SIM_STATES.map(([id, label]) => (
            <button key={id} onClick={() => {
              ...
              background: state===id ? 'rgba(35,22,32,0.95)' : 'transparent',
              border:`1px solid ${state===id?'#3A1826':T.border}`,
              borderRadius:5, padding:'7px 4px',
              color: state===id ? '#7A4860' : T.txt3,
      ...
      {/* ── Footer ─────────────────────────────────────────────────── */}
      <div style={{
        marginTop:20, fontSize:7, letterSpacing:'0.28em',
        color:'#14121C', textAlign:'center',
      }}>
